...
docker-clean-dev:
	-@docker rmi admindaspanel/rootfs-base-dev

docker-build-dev: clean docker-clean-dev rootfs-fixperms
	docker build -f Dockerfile.dev -t admindaspanel/rootfs-base-dev .
...
patch-release: guard-GITHUB_TOKEN clean check-status check-release build-zip build-dist
	...
	tools/github-release release -u daspanel -r rootfs-base -t $(NEXT_PATCH) -n $(NEXT_PATCH) -d "**Changelog**<br/>$$changelog"
	@DIST_FILES="$(shell ls dist/)"; \
	echo "$$DIST_FILES"; \
	for i in $$DIST_FILES; \
	do \
		echo "Uploading $$i..."; \
		tools/github-release upload -u daspanel -r rootfs-base -t $(NEXT_PATCH) -n $$i -f dist/$$i -R; \
	done
...
minor-release: guard-GITHUB_TOKEN clean check-status check-release build-zip build-dist
	...
	tools/github-release release -u daspanel -r rootfs-base -t $(NEXT_MINOR) -n $(NEXT_MINOR) -d "**Changelog**<br/>$$changelog"
	@DIST_FILES="$(shell ls dist/)"; \
	echo "$$DIST_FILES"; \
	for i in $$DIST_FILES; \
	do \
		echo "Uploading $$i..."; \
		tools/github-release upload -u daspanel -r rootfs-base -t $(NEXT_MINOR) -n $$i -f dist/$$i -R; \
	done
...
major-release: guard-GITHUB_TOKEN clean check-status check-release build-zip build-dist
	...
	tools/github-release release -u daspanel -r rootfs-base -t $(NEXT_MAJOR) -n $(NEXT_MAJOR) -d "**Changelog**<br/>$$changelog"
	@DIST_FILES="$(shell ls dist/)"; \
	echo "$$DIST_FILES"; \
	for i in $$DIST_FILES; \
	do \
		echo "Uploading $$i..."; \
		tools/github-release upload -u daspanel -r rootfs-base -t $(NEXT_MAJOR) -n $$i -f dist/$$i -R; \
	done
